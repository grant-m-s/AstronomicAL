...
def spectrum_plot(data, selected = None, plot_instance = None, dataset = "DESI", from_specid = False):
    ...
    def update_plot(new_radius, panel_id = None):
        ...
        if desi_object is not None:
            ...
            plot = get_desi_figure_hv(desi_object, plot_emlines = (not is_star), plot_abslines=is_star)
...
def get_desi_figure_hv(desi_object, plot_emlines= True, plot_abslines = True):
    N = desi_object.available_spectra
    nrows, ncols = N, 1
    hv_plots = []
    colors = plt.get_cmap("gist_rainbow", max(N,2))
    for i in range(N):
        plot = desi_object.plot_spectrum_hv(
            idx=i,
            show_xlabel = True,
            show_ylabel = True,
    ...
    desi_fig = hv.Layout(hv_plots).cols(ncols)
    desi_fig = desi_fig.opts(
...
#class EuclidPanelManager_matplotlib:
#
#    """A class that handles the retrieve of Euclid cutouts and its manipulation with 
#    the widgets"""
#    
...
#     
#    def _update_radius(self, event):
#        if event.new: #avoid passing nan/None
#            self.radius = event.new
#            shared_data.publish(self.panel_id, "Euclid_radius", self.radius)
#            with concurrent.futures.ThreadPoolExecutor() as executor:
#                future = executor.submit(self.run_euclid, initialize = False)
