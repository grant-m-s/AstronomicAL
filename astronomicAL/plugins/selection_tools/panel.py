...
def _rows_from_source_by_selection_ids(
    ...
    *,
    ...
    selected_ids = [str(row_id) for row_id in (row_ids or [])]

    if not selected_ids:
        return pd.DataFrame(columns=list(selected_columns or []))

    print(
        f"[SelectionSet] preview lookup rows={len(selected_ids)} "
        f"source={type(source).__name__} batched={callable(getattr(source, 'get_rows_by_ids', None))}",
        flush=True,
    )

    if id_col and id_col != "Use Index":
        get_rows_by_ids = getattr(source, "get_rows_by_ids", None)
        if callable(get_rows_by_ids):
            try:
                preview = get_rows_by_ids(
                    selected_ids,
                    id_column=id_col,
                ...
                # Make sure row ids are present for display/focus matching.
                if id_col in preview.columns:
                    preview["_selection_row_id"] = preview[id_col].astype(str)
                else:
                    preview["_selection_row_id"] = selected_ids[: len(preview)]

                return preview
            except Exception:
                traceback.print_exc()

    # Fallback path for sources without vectorised lookup.
...
class SelectionSetPanel:
    ...
    def __init__(self, context, data=None):
        ...
        self.preview_table.object = "Loading selection preview..."
        self._schedule_refresh_after_open()
    ...
    def _schedule_refresh_after_open(self) -> None:
        """Refresh after the panel has had a chance to attach to a Bokeh doc.

        Important: do not run the heavy refresh synchronously from __init__.
        """
        ...
        try:
            pn.state.onload(_runner)
            return
        except Exception:
            traceback.print_exc()

        # Last resort: leave the panel alive rather than blocking/crashing creation.
        self.status_pane.object = (
            "Selection preview will refresh after the next selection or dataset event."
        )
    ...
    def _state_signature(self):
        ...
        row_ids = getattr(state, "row_ids", None) if state is not None else None
        row_count = len(row_ids) if row_ids is not None else 0
        ...
        return (
            ...
            row_count,
            getattr(state, "timestamp", None) if state is not None else None,
    ...
    def schedule_refresh(self) -> None:
        ...
        # Do not synchronously refresh from event callbacks / construction.
        self._refresh_pending = False
    ...
    def refresh(self, *, force: bool = False) -> None:
        ...
        print(f"[SelectionSet] signature {time.perf_counter() - t0:.3f}s", flush=True)
    ...
    def _refresh_panel_state(self) -> None:
        ...
        raw_row_ids = getattr(state, "row_ids", []) or []
        row_count = len(raw_row_ids)
        preview_row_ids = [str(r) for r in raw_row_ids[:SELECTION_PREVIEW_LIMIT]]        
        ...
        print(f"[SelectionSet] preview {time.perf_counter() - t0:.3f}s", flush=True)
