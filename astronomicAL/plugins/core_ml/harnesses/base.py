...
from ..split_manifest import canonical_json, json_scalar, update_canonical_digest
...
class RunHarness:
    ...
    def _resume_partition_identity(
        ...
        """Return only stable semantic partition identity.

        Physical manifest paths and compressed-file checksums are intentionally
        excluded. Every non-empty partition must provide membership, target,
        source-revision, protocol, and algorithm identity. Checkpoints lacking
        those fields are not safe to resume.
        """
        ...
        for role, raw_signature in dict(signatures or {}).items():
            ...
            if not isinstance(raw_signature, Mapping):
                raise ValueError(
                    f"Partition identity for {role_name!r} is not a mapping."
                )
            ...
            missing = sorted(required_fields - set(signature))
            if missing:
                raise ValueError(
                    "Resume checkpoint does not contain stable partition "
                    f"identity for {role_name!r}. Missing fields: {missing!r}."
                )

            normalized[role_name] = {
                "count": int(signature["count"]),
                "role": str(signature.get("role") or role_name),
                "source_dataset_id": (
                    str(signature["source_dataset_id"])
                    if signature.get("source_dataset_id") is not None
                    else None
                ),
                "identity_version": int(signature["identity_version"]),
                "digest_algorithm": str(signature["digest_algorithm"]),
                "membership_sha256": str(signature["membership_sha256"]),
                "target_sha256": str(signature["target_sha256"]),
                "source_revision_sha256": str(
                    signature["source_revision_sha256"]
                ),
                "protocol_id": str(signature["protocol_id"]),
            }
    ...
    def _validate_resume_identity(
        ...
        if not saved_signatures:
            raise ValueError(
                "Resume checkpoint is missing stable partition and source "
                "identity. Resume was refused."
            )
        ...
        if saved_identity != current_identity:
            ...
            for role in differing_roles:
                saved_role = saved_identity.get(role)
                current_role = current_identity.get(role)
                if not isinstance(saved_role, Mapping) or not isinstance(
                    current_role,
                    Mapping,
                ):
                    details.append(
                        f"{role}: saved={saved_role!r}, "
                        f"current={current_role!r}"
                    )
                    continue
                role_fields = sorted(
                    key
                    for key in set(saved_role) | set(current_role)
                    if saved_role.get(key) != current_role.get(key)
                )
                differing_fields.extend(
                    f"{role}.{field}" for field in role_fields
                )
                details.append(
                    f"{role}: "
                    + ", ".join(
                        (
                            f"{field} saved={saved_role.get(field)!r} "
                            f"current={current_role.get(field)!r}"
                        )
            ...
            raise ValueError(
                "The dataset identity no longer matches the paused run. "
                "Resume was refused because partition membership, targets, "
                "ML-relevant source contents, or source roles changed. "
    ...
    def _partition_signatures(self, parts: Partitions) -> Dict[str, Any]:
        ...
        for role, partition in (
            ...
            source_dataset_id = self._logical_partition_dataset_id(role)
            if isinstance(partition, PartitionRef):
                source_revision = partition.manifest.source_revision(
                    partition.dataset_id
                )
                membership_sha256 = (
                    partition.membership_sha256
                    or partition.manifest.membership_sha256_by_role.get(
                        partition.role,
                        "",
                    )
                )
                target_sha256 = (
                    partition.target_sha256
                    or partition.manifest.target_sha256_by_role.get(
                        partition.role,
                        "",
                    )
                )
                source_revision_sha256 = (
                    partition.source_revision_sha256
                    or (
                ...
                if partition.manifest.target_column and not target_sha256:
                    missing.append("target_sha256")
                if missing:
                    raise ValueError(
                        f"Partition {role!r} is missing stable identity "
                        f"field(s): {missing!r}."
                    )
                ...
                result[role] = {
                    ...
                    "identity_version": int(partition.identity_version),
                    "digest_algorithm": str(partition.digest_algorithm),
                    "membership_sha256": str(membership_sha256),
                    "target_sha256": str(target_sha256),
                    "source_revision_sha256": str(
                        source_revision_sha256
                    ),
                    "protocol_id": str(parts.protocol_id),
                }
                continue

            if not isinstance(partition, Partition):
                raise TypeError(
                    f"Unsupported partition type for {role!r}: "
                    f"{type(partition)!r}."
                )

            rows = self._canonical_partition_rows(partition)
            ...
            frame_key = "val" if role == "validation" else role
            frame = self._partition_frames.get(frame_key)
            if frame is None:
                raise ValueError(
                    f"Partition frame for {role!r} is unavailable; stable "
                    "source identity cannot be calculated."
                )
            ...
            result[role] = {
                ...
                "source_revision_sha256": (
                    self._materialized_source_revision_sha256(
                        frame,
                        source_dataset_id=source_dataset_id,
                    )
                ),
    ...
    def _logical_partition_dataset_id(
        self,
        role: str,
    ) -> Optional[str]:
        if role == "train":
            return str(self.run.dataset_id)
        if role == "validation":
            if self.protocol.validation_source == "dataset":
                return (
                    str(self.protocol.validation_dataset_id)
                    if self.protocol.validation_dataset_id is not None
                    else None
                )
            return str(self.run.dataset_id)
        if role == "test":
            if self.protocol.test_source == "none":
                return None
            if self.protocol.test_source == "dataset":
                return (
                    str(self.protocol.test_dataset_id)
                    if self.protocol.test_dataset_id is not None
                    else None
                )
            return str(self.run.dataset_id)
        raise KeyError(f"Unknown partition role {role!r}.")

    @staticmethod
    def _canonical_partition_rows(
        partition: Partition,
    ) -> List[tuple[Any, Any]]:
        if len(partition.labels) not in {0, len(partition.record_ids)}:
            raise ValueError(
                f"Partition {partition.name!r} has mismatched record and "
                "target counts."
            )
        rows = [
            (
                ...
                (
                    json_scalar(partition.labels[index])
                    if partition.labels
                    else None
                ),
            )
            for index, record_id in enumerate(partition.record_ids)
        ]
        rows.sort(key=lambda row: canonical_json(row[0]))
        return rows

    def _materialized_source_revision_sha256(
        self,
        frame: Any,
        *,
        source_dataset_id: Optional[str],
    ) -> str:
        columns = self._identity_columns_for_frame(frame)
        dtypes = {
        ...
        for raw_row in frame.loc[:, columns].itertuples(
            index=False,
            name=None,
        ):
            values = dict(zip(columns, raw_row))
            record_id = json_scalar(
                values[self.binding.record_id_column]
            )
            ...
            rows.append(
                (
                    canonical_json(record_id),
                    row_digest.hexdigest(),
                )
            )

        rows.sort(key=lambda row: row[0])
        for record_id_key, row_sha256 in rows:
            update_canonical_digest(
                digest,
                record_id_key,
                row_sha256,
    ...
    def _identity_columns_for_frame(self, frame: Any) -> List[str]:
        columns = [self.binding.record_id_column]
        if self.binding.target_column:
            columns.append(self.binding.target_column)
        columns.extend(
            str(column)
            for column in (self.binding.input_columns or [])
            if str(column).strip()
        )
        if self.binding.image_column:
            columns.append(self.binding.image_column)
        if self.protocol.group_column:
            columns.append(self.protocol.group_column)
        if self.protocol.split_column:
            columns.append(self.protocol.split_column)
        columns = sorted(
            dict.fromkeys(str(column) for column in columns if column)
        )
        missing = [
        ...
        if missing:
            raise ValueError(
                "Partition frame is missing ML identity column(s): "
                f"{missing!r}."
            )
        return columns
...
# ---- the protocol flow (NOT overridable by recipes) --------------------
    def execute(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Subclass model writers predate the canonical class-name contract and
            # may use aliases such as ``classes`` or only retain label_options in
            # checkpoint params. Promote the harness-owned target ordering onto the
            # durable model artifact before any automatic post-training prediction.
            from ..artifacts import persist_model_class_contract

            persist_model_class_contract(
...
# ---- partitioning (modality-agnostic) ----------------------------------
    ...
    def _write_split_spec(self, parts: Partitions) -> Optional[str]:
        def inline_row_ids(partition: Any) -> List[str]:
            if isinstance(partition, Partition):
                return [str(value) for value in partition.record_ids]
            return []

        partition_identity = self._partition_signatures(parts)
        return self.run.put_artifact(
            "ml.split_spec",
            {
                ...
                "validation_dataset_id": parts.validation_dataset_id,
                "test_dataset_id": parts.test_dataset_id,
                "split_dataset_ids": dict(
                    parts.materialized_split_dataset_ids or {}
                ),
                ...
                "train_row_ids": inline_row_ids(parts.train),
                "validation_row_ids": inline_row_ids(parts.val),
                "test_row_ids": inline_row_ids(parts.test),
                "row_ids_inline_complete": all(
                    partition is None
                    or isinstance(partition, Partition)
                    for partition in (
                        parts.train,
                        parts.val,
                        parts.test,
                ...
                "split_manifest": (
                    ...
                    if parts.split_manifest is not None
                    else None
                ),
                "partition_refs": {
                    role: ref.to_dict()
                    for role, ref in dict(
                        parts.partition_refs or {}
                    ).items()
                },
                ...
                "isolation": {
                    ...
                    "explicit_split_datasets": bool(
                        parts.materialized_split_dataset_ids
                    ),
                    "stable_membership_identity": True,
