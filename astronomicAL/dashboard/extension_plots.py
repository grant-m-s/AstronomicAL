...
def create_plot(data, x, y, plot_type="scatter", selected=None, label_plot=True, colours=True):
    ...
    if selected is not None:
        ...
        if len(selected.data[cols[0]]) == 1:
            selected = pd.DataFrame(
                    selected.data, columns=cols, index=[0])
        else:
        ...
        plot = (
            dynspread(
                datashade(
                    ...
                    color_key=color_key,
                    aggregator=ds.by(
                        config.settings["label_col"], ds.count()),
...
def agn_wedge(data):
    ...
    plot = create_plot(
            data, "Log10(W3_Flux/W2_Flux)", "Log10(W2_Flux/W1_Flux)", plot_type="scatter", label_plot=False)
    ...
    threshold_y = threshold_w[(top_y_orig > threshold_w) & (
            bottom_y_orig < threshold_w)]
    threshold_x = x[(top_y_orig > threshold_w)
                    & (bottom_y_orig < threshold_w)]

    top_x = np.array([np.min(top_x), np.max(top_x)])
    top_y = (0.315 * top_x) + 0.297
    top = pd.DataFrame(
            np.array([top_x, top_y]).transpose(), columns=["x", "y"])
    ...
    p1 = create_plot(
            top, "x", "y", plot_type="line", label_plot=False, colours=False)
    p2 = create_plot(
